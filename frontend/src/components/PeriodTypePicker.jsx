...
export default function PeriodTypePicker() {
  ...
  return (
    <div className="flex gap-2 flex-wrap items-center">
      <select className="field-input py-1.5 w-auto" value={perfPeriod.type} onChange={(e) => setType(e.target.value)}>
        {PERIOD_TYPES.map((t) => <option key={t} value={t}>{PERIOD_TYPE_LABEL[t]}</option>)}
      </select>
      {perfPeriod.type === 'monthly' && (
        <select className="field-input py-1.5 w-auto" value={perfPeriod.idx} onChange={(e) => setIdx(e.target.value)}>
      ...
      <select className="field-input py-1.5 w-auto" value={perfPeriod.year} onChange={(e) => setYear(e.target.value)}>
