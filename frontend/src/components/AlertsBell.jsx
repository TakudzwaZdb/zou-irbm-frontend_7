...
// What DOES clear once opened and read is the badge: the red count is only
// ever the alerts this account hasn't opened the bell to see yet (see
// lib/seenAlerts.js) — opening the dropdown marks everything currently
// shown as seen, so the badge goes quiet until something genuinely new
// shows up, without ever pretending a still-open issue went away.
  ...
  function toggleOpen() {
    setOpen((wasOpen) => {
      ...
      if (willOpen && alerts.length > 0) {
        ...
        alerts.forEach((a) => merged.add(signature(a)));
        setSeen(merged);
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto rounded-xl border border-line bg-surface shadow-lg z-50">
            ...
            {alerts.length === 0 ? (
              ...
              alerts.map((a) => (
                <button
                  key={a.id}
                  onClick={() => { setRoute(a.route); setOpen(false); }}
                  className="w-full text-left px-3.5 py-2.5 border-b border-line last:border-0 hover:bg-sunken flex flex-col gap-1"
                >
